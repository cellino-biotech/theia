...
def RunGaussianProcess(Ividmeas, zfocus, z_vec, lambd, ps, Nsl, eps1, eps2, reflect):
    ...
    I0 = Ividmeas[:, :, zfocus]     # isolate focal layer
    zfocus = z_vec[zfocus]          # convert zfocus from index to real dimension

    ### Calculate S_c ###

    # why is dz=1
    freqs = CalFrequency(Ividmeas[:, :, 0], lambd, ps, np.abs(z_vec[-1] - z_vec[-2]))
    ...
    for k in range(Nsl):
        ...
        # NOTE: dIdz is returned as a multidimensional array of zeros
        # Coeff equals [0, 0, 0]
        # Coeff2 equals [0, 1, 0]
...
def CalFrequency(img, lambd, ps, dz):
    ...
    # find real image dimensions using pixel lengths and pixel scale (um)
    dfx = 1 / nx / ps
    dfy = 1 / ny / ps

    (Kxdown, Kydown) = np.mgrid[-nx // 2 : nx // 2, -ny // 2 : ny // 2]

    # TODO: learn about Kxdown and Kydown
    Kxdown = Kxdown * dfx
    Kydown = Kydown * dfy
...
def CombinePhase(dIdzStack, Frq_cutoff, freqs, CoeffStack, Coeff2Stack):
   ...
   #print(freqs.shape, dIdzStack.shape)
    ...
    for k in range(Nsl):
        ...
        f1 = Frq_cutoff[k]
        Maskf = np.zeros((Nx, Ny))
        # print(freqs.shape, Maskf.shape, f1, f0, np.sum((freqs <= f1) & (freqs > f0)))
