...
#       fov_mm = 1544 * pixel_size_mm (pixel_size_um = 0.35)
...
EXPOSURE_TIME_US = 1500  # at max light intensity
FPS_MAX = 635
...
def set_roi_zones(camera: object, num_zones: int = 3):
    ...
    spacing = math.trunc((SENSOR_HEIGHT_PIX-4) / (num_zones-1))
    offset = 0

    for i in range(num_zones):
        camera.ROIZoneSelector.SetValue(f"Zone{i}")
        try:
            camera.ROIZoneOffset.SetValue(offset)
        except genicam.OutOfRangeException:
            # camera offset has to be divisible by minimum zone pix height
    ...
    # use queried framerate to set upper limit
