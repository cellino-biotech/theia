...
def check_img_format(img_arr: ndarray):
    ...
    proc_arr = [img_arr[i] for i in range(img_arr.shape[0])]

    # initialize for when layers are cropped and reassembled
    crop_arr = None


def row_shift(img_arr: ndarray, plot: bool = False):
    for i in range(int(len(proc_arr) / 2)):
...
def crop(self):
    ...
    for i in range(len(self.proc_arr)):
        if self.proc_arr[i].shape[0] > shape[0]:
            ...
            if diff % 2:
                self.proc_arr[i] = self.proc_arr[i][int(diff / 2) + 1 : int(-diff / 2)]
            else:
